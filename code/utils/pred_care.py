...
def to_tensor(x: np.ndarray, channel: Optional[int] = None, single_sample: bool = True) -> torch.Tensor:
    """
    Convert numpy array to PyTorch tensor with proper channel handling.

    Args:
        x: Input numpy array
        channel: Channel dimension index (negative values count from end)
        single_sample: Whether to add batch dimension if input is single sample

    Returns:
        PyTorch tensor with proper channel ordering
    """
    if single_sample:
        x = x[np.newaxis]
    ...
    # Move channel to expected position (PyTorch uses channel-first by default)
    if channel != 1 and x_tensor.dim() > 1:
        dims = list(range(x_tensor.dim()))
        dims.insert(1, dims.pop(channel))
        x_tensor = x_tensor.permute(*dims)

    return x_tensor


def from_tensor(x: torch.Tensor, channel: int = -1, single_sample: bool = True) -> np.ndarray:
    """
    Convert PyTorch tensor back to numpy array with original channel ordering.

    Args:
        x: Input PyTorch tensor
        channel: Desired channel dimension in output
        single_sample: Whether to remove batch dimension if output is single sample

    Returns:
        Numpy array with original channel ordering
    """
    if single_sample:
        x = x[0]

    # Move channel from PyTorch default (channel-first) to desired position
    if channel != 0 and x.dim() > 0:
        current_channel_dim = 0 if x.dim() > 1 else None
        if current_channel_dim is not None:
            dims = list(range(x.dim()))
            dims.insert(channel, dims.pop(current_channel_dim))
            x = x.permute(*dims)

    return x.numpy()


def tensor_num_channels(x: torch.Tensor) -> int:
    """Get number of channels in tensor (PyTorch uses channel-first by default)."""
    return x.shape[1] if x.dim() > 1 else 1
...
def predict_direct(
    ...
    Args:
        model: PyTorch model
        x: Input numpy array
        axes_in: Input axes specification (e.g. 'SZYXC')
        axes_out: Output axes specification (defaults to axes_in)
    ...
    ax_in = {a: i for i, a in enumerate(axes_in)}
    ax_out = {a: i for i, a in enumerate(axes_out)}

    channel_in, channel_out = ax_in['C'], ax_out['C']
    single_sample = 'S' not in ax_in

    # Convert to tensor
    x_tensor = to_tensor(x, channel=channel_in, single_sample=single_sample)

    # Predict
    with torch.no_grad():
        pred_tensor = model(x=x_tensor.to('cuda'),
                            coord=grid[0][0].to('cuda'),
                            cell=grid[1][0].to('cuda'),
                            prompt_tensor=None,#prompt,
                            prompt_enable=False,
                            s_factor=1
                            )

    # Convert back to numpy
    pred = from_tensor(pred_tensor.cpu(), channel=channel_out, single_sample=single_sample)
...
def predict_tiled(
        model: torch.nn.Module,
        x: np.ndarray,
        n_tiles: Union[int, List[int]],
        block_sizes: Union[int, List[int]],
        tile_overlaps: Union[int, List[int]],
        axes_in: str,
        grid: list,
        axes_out: Optional[str] = None,
    ...
    """
    Tiled prediction for large inputs.

    Args:
        model: PyTorch model
        x: Input numpy array
        n_tiles: Number of tiles per dimension
        block_sizes: Block sizes per dimension
        tile_overlaps: Tile overlaps per dimension
        axes_in: Input axes specification
        axes_out: Output axes specification (defaults to axes_in)
    ...
    if np.isscalar(tile_overlaps):
        tile_overlaps = [tile_overlaps] * x.ndim

    if all(t == 1 for t in n_tiles):
        pred = predict_direct(model=model, x=x, axes_in=axes_in, grid=grid, axes_out=axes_out, **kwargs)
        if pbar is not None:
            pbar.update()
        return pred
    ...
    axes_in = axes_check_and_normalize(axes_in, x.ndim)
    ...
    def _to_axes_out(seq: List, elem: Union[int, slice]) -> Tuple:
        """Convert sequence from input axes to output axes semantics."""
        assert len(seq) == len(axes_in)
        seq = [seq[ax_in[a]] for a in axes_out]
        seq[ax_out['C']] = elem
        return tuple(seq)

    assert x.ndim == len(n_tiles) == len(block_sizes) == len(tile_overlaps)
    assert n_tiles[channel_in] == 1
    assert all(n_tiles[ax_in[a]] == 1 for a in axes_lost)
    assert all(isinstance(t, int) and t >= 1 for t in n_tiles)

    # Find first axis with more than 1 tile
    ...
    for tile, s_src, s_dst in tile_iterator_1d(
            x, axis=axis, n_tiles=n_tiles[axis],
            block_size=block_size, n_block_overlap=n_block_overlap
    ):
        pred = predict_tiled(
            model, tile, n_tiles_remaining, block_sizes,
            tile_overlaps, axes_in,grid, axes_out, pbar=pbar, **kwargs
        )
...
# Tile iterator functions (unchanged from original)
